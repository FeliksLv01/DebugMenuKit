...
2. `Package.swift` must expose the `DebugMenuKit` library product and use the local `Macros` package.
...
1. Macro declarations belong in `Macros/Sources/DebugMenuKitMacro`.
2. Compiler-plugin implementations belong in `Macros/Sources/DebugMenuKitMacros`.
